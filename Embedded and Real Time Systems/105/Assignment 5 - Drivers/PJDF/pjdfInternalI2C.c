...
static PjdfErrCode ReadI2C(DriverInternal *pDriver, void* pBuffer, INT32U* pCount)
{
    ...
    I2C_start(pContext->i2cMemMap, pContext->i2CDevAddr, I2C_Direction_Receiver);
    INT32U i;
    //read a byte and request the next byte
    for(i = 0; i < (*pCount)-1; i++) { 
      ((INT8U*)pBuffer)[i] = I2C_read_ack(pContext->i2cMemMap);
    }
    //read last byte and generate stop signal
    ((INT8U*)pBuffer)[i] = I2C_read_nack(pContext->i2cMemMap);
    return PJDF_ERR_NONE;
}
...
static PjdfErrCode WriteI2C(DriverInternal *pDriver, void* pBuffer, INT32U* pCount)
{
    PjdfContextI2c *pContext = (PjdfContextI2c*) pDriver->deviceContext;
    if (pContext == NULL) while(1);
    I2C_start(pContext->i2cMemMap, pContext->i2CDevAddr, I2C_Direction_Transmitter);
    //send address int the first byte
    I2C_write(pContext->i2cMemMap, ((INT8U*)pBuffer)[0]);
    //write additional data if necessary
    INT32U i;
    for(i = 0; i < (*pCount); i++) { 
      I2C_write(pContext->i2cMemMap, ((INT8U*)pBuffer)[i+1]);
    }
